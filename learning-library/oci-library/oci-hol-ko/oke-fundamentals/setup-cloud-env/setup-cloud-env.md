...
운영 엔지니어의 역할을 맡게 됩니다. 마이크로서비스 애플리케이션을 배포하는 데 사용할 Oracle 클라우드 환경을 만듭니다. 이 환경은 클라우드 구획 내에 포함되며 구획 내 통신은 가상 클라우드 네트워크(VCN)를 통해 이루어집니다. 구획과 VCN은 전체 환경을 격리하고 보호합니다. OCI Kubernetes Engine(OKE)으로 Kubernetes 클러스터를 만들것입니다.
...
1. **oci-hol**-*xx* Compartment에 있는지 먼저 확인하고 **Create Cluster**을 클릭 합니다.
...
1. 팝업이 뜨면 확인 후 Continue를 클릭합니다. 여기서 체크하지 않으면, 기본적으로 Enhanced Cluster 타입이 선택됩니다.

    ![Basic Cluster Confirmation](images/basic-cluster-confirmation-unchecked.png =60%x*)

1. 기본 자원이 생성되는 것을 확인 후 Close를 클릭합니다.
...
5. **Access command**를 복사하여 Cloud Shell 터미널에 붙여 실행하여 kubeconfig을 만듭니다. Cloud Shell은 OCI 콘솔에 로그인한 유저로 인증되어 해당 유저의 권한을 가집니다.

    * 로컬 접근하려면, Local access 절차에 따라 OCI CLI 설치 및 인증 설정을 먼저 진행하고, kubeconfig 파일을 생성해야 합니다.
